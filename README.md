...
The patient interview now uses a large animated SVG assistant and a hands-free speaking/listening loop. After each spoken question, the microphone opens automatically; voice activity and sustained silence determine when the answer finishes. Real PCM audio is transcribed by local Whisper and sent through the existing clinical engine. The server asks **5–10 adaptive questions**, with a hard stop at ten. A browser may require **Start** once and microphone permission; no button is required between successful voice turns. Optional transcript correction, typing, pause and help remain available.

Piper playback amplitude drives the mouth. Idle breathing/blinking, attentive listening and thinking/head movement indicate the assistant's state. Continuous AudioWorklet capture preserves long answers across ASR chunks. Brief pauses are tolerated; silence produces a gentle prompt and unclear speech produces an automatic retry. Red flags pause the normal conversation immediately for human help. Existing report information can be confirmed naturally instead of being re-entered. The final review and consent remain explicit. See [hands-free architecture, configuration and verification](docs/HANDSFREE_INTERVIEW.md).

If Piper fails, an installed local browser voice is used if available, with speech-boundary-driven approximate mouth motion. Otherwise the question stays readable and typing remains available. This fallback does not silently use a remote browser voice. Browser tests inject synthetic PCM into Chromium's actual audio pipeline; they do not certify the physical microphone or speakers. Use Chrome or Edge on localhost/HTTPS for the live demonstration. Hindi uses the local Whisper small model; English uses base by default.
