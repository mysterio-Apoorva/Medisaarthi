...
export function AssignmentPanel({users}:{users:AdminUser[]}) {
  ...
  useEffect(()=>{ fetch(`${API_BASE_URL}/admin/providers`,{credentials:'include'}).then(async response=>{if(response.ok)setProviders(await response.json());}).catch(()=>{}); },[]);
  return <form className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3" onSubmit={async e=>{
    ...
    <details><summary className="text-sm cursor-pointer font-semibold">AI provider configuration and observed health</summary><pre className="text-xs whitespace-pre-wrap mt-2">{providers ? JSON.stringify(providers,null,2) : 'Provider status has not loaded.'}</pre></details>
  </form>;
}
