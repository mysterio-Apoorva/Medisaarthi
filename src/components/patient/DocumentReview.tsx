...
function failureReason(code: string) {
  if (code === 'NO_MACHINE_READABLE_TEXT') return 'Unable to extract readable text. Review the original document or upload a clearer scan.';
  if (code.includes('UNAVAILABLE')) return 'The document text/OCR engine is unavailable. Retry processing when the service is restored.';
  return `Document processing failed (${code}). Retry or review the original with your clinician.`;
}

export function DocumentReview({ encounterId, allowUpload = false, onChange, recordRevision }: { encounterId: string; allowUpload?: boolean; onChange?: () => void; recordRevision?: number }) {
  ...
  const load = useCallback(() => listEncounterDocuments(encounterId).then(setDocuments), [encounterId]);
  useEffect(() => { load().catch(reason => setError(reason instanceof Error ? reason.message : 'Unable to load documents.')); }, [load, recordRevision]);
  ...
  const uploadMany = async (files: FileList | null) => {
    ...
    try { await load(); onChange?.(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to reload saved documents'); }
  };
  ...
  const retry = async (documentId: string) => {
    try {
      ...
      await load(); await inspect(documentId); onChange?.();
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4" aria-labelledby="documents-heading">
    ...
    {Object.entries(jobs).filter(([, value]) => !['Completed', 'Needs review'].includes(value)).map(([key, value]) => <p key={key} role="status" className="flex items-center gap-2 rounded-xl bg-sky-50 p-3 text-sm text-sky-950">{value === 'Failed' ? <TriangleAlert className="h-4 w-4" /> : <LoaderCircle className="h-4 w-4 animate-spin" />}{key.split('-')[0]}: {value}</p>)}
    ...
    <div className="space-y-2">
      {documents.map(document => <article key={document.document_id} className="rounded-xl border border-slate-200 p-4">
        ...
        {document.error_code && <p role="alert" className="mt-2 text-sm text-rose-800">{failureReason(document.error_code)}</p>}
        <div className="mt-3 flex flex-wrap gap-3 text-sm font-semibold">
          ...
          {document.can_retry && <button type="button" disabled={jobs[document.document_id] === 'Extracting text'} className="inline-flex items-center gap-1 text-sky-700 underline disabled:opacity-50" onClick={() => void retry(document.document_id)}><RefreshCw className="h-3.5 w-3.5" /> Retry</button>}
          {allowUpload && document.can_remove && <button type="button" className="inline-flex items-center gap-1 text-rose-700 underline" onClick={() => void remove(document.document_id)}><Trash2 className="h-3.5 w-3.5" /> Remove</button>}
        </div>
      </article>)}
    </div>
    {selected && <div className="rounded-2xl border border-sky-200 bg-sky-50/40 p-4 space-y-4" aria-live="polite">
      ...
      {selected.error_code && <p role="alert" className="text-sm text-rose-800">{failureReason(selected.error_code)}</p>}
